...
describe('AuthService', () => {
  ...
  it('새 계정 생성시 사용중인 이메일 사용 테스트', async () => {
    ...
    await expect(service.signup('test22@test.com', '1234')).rejects.toThrow(
      BadRequestException,
    );
  });

  it('로그인 시 없는사용자 테스트', async () => {
    await expect(service.signin('dsdsd@test.com', '1234')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('틀린 비밀번호 입력 테스트', async () => {
    ...
    await expect(
      service.signin('test22@test.com', 'mypassword'),
